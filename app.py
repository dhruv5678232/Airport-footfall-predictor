...
# Flight type logic (assuming dataset has domestic_flights and international_flights)
if 'domestic_flights' in df.columns and 'international_flights' in df.columns:
    ...
    st.warning("Dataset lacks 'domestic_flights' and 'international_flights'. Using fallback logic.")
    df['flight_type'] = (df.get('domestic_flights', 0) == 0).astype(int)
...
temperature_min = int(df["temperature"].min()) if "temperature" in df.columns else 0
temperature_max = int(df["temperature"].max()) if "temperature" in df.columns else 40
...
mean_total_flights = int(df["total_flights"].mean()) if "total_flights" in df.columns else 100
mean_load_factor = df["load_factor"].mean() if "load_factor" in df.columns else 0.75
mean_economic_trend = df["economic_trend"].mean() if "economic_trend" in df.columns else 0
...
df_encoded["weather_good"] = df_encoded["weather_good"].astype(int) if "weather_good" in df_encoded.columns else 1
...
# Check for missing columns and train model
missing_cols = [col for col in features + [target] if col not in df_encoded.columns]
if missing_cols:
...
flight_fare_adjustment = 1.0 if selected_flight_type == "Domestic" else 1.5  # International 50% higher
...
export_data = pd.DataFrame({
    ...
    "Future Predicted Footfall": [future_footfalls[-1]]
})
